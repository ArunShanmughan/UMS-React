...
import { ADMIN, PASSWORD, JWT_KEY } from "../utils/config.js";
...
export const adminLogin = async (req, res) => {
  try {
    ...
    if (email === ADMIN && password === PASSWORD) {
      ...
      return res
        .status(401)
        .send({ message: "Invalid email or password.", passVer: true });
    }
  } catch (err) {
    console.log(err);
    return res
      .status(500)
      .send({ message: "An error occurred during login.", AdminVer: false });
  }
};
